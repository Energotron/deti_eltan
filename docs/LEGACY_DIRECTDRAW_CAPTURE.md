...
`Rangers.exe` использует старый полноэкранный графический путь. Стандартный захват
...
Последующая проверка уточнила причину первой ошибки: Steam HD build фактически
загружает системный D3D9, а Windows Graphics Capture helper вызывает
`IsBorderRequired`, доступный с Windows build 20348, на установленном build
19045. Проверенный `cnc-ddraw` не загрузился и был полностью удалён.
